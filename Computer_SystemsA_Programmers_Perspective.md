...
​	一个简单但很有用的组合电路，称为**多路复用器**，在单个位的多路复用器中，两个数据信号是输入 a 和 b ，控制信号输入是 s 。当 s 是 1 时，输出为 a ，反之输出为 b 。值得注意的是，在**硬件控制语言（ HCL ）**中的逻辑表达式中是没有短路求值的。

  ```c++
bool out = (s && a) || (!s && b);
  ```

​	类似地，我们也可以设计出字级的多路复用器，由多个相同的子电路组成。
